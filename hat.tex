...
involved in the electron transport chain which are responsible for radical
...
produce excess radicals leading to oxidative damage which has been linked to
...
does have an influence on the ability of a metal cation to effect HAT reactions.
...
reactivity between \cumo\ and DMSO, as well as the radical H-atom donation
reactivity between \bno\ and DMSO. I also performed calculations to determine
the effects of metal cations and if there exists reverse reactivity for two
other strong Lewis basic substrates: hexamethylphosphoramide (HMPA) and
tributylphosphine oxide (TBPO). The chemical structures of all the species
studies herein are shown in~\ref{fig:hat-subs}.
...
required for the resolution-of-the-identity (RI) approximation as implemented in
TURBOMOLE. The RI approximation is used to reduce the computational cost
...
6-31+G(2d,2p) basis sets were tested both by single-point energy calculations on
the benchmark structures. Geometry optimization calculations starting from the
benchmark structures were also performed for three of the best performing
DFT-based method, in order to verify their ability to capture the minimum energy
bound structures.
...
optimizations were performed at the M05-2X\cite{Zhao2006}/6-31+G$^{**}$ level of
...
Suarez2011, Siu2001, Baldauf2013} Most importantly, benchmark quality data for a
wide variety of metals binding to biologically relevant substrates and
...
benchmark quality calculations on the originally proposed benchmark set were not
possible. Full details of the originally proposed benchmark set are in
...
Appendix~\ref{ap:hat}, ~\ref{fig:ap_pes_metals}). Examining the energy of each
ion relative to the smallest basis set, for \ch{Li^+} the value appears to
converge reasonably, however this because there are only 2 electrons in this
ion. For all of \ch{Na^+}, \ch{Mg^{2+}}, \ch{K^+}, and \ch{Ca^{2+}}, there
appears to be no convergence to the CBS limit as each line continues down
linearly. This is problematic as it means that CBS extrapolation would result in
a significant degree of uncertainty in the estimated CBS limit total energy.
...
~\ref{fig:ap_pes_metals_2}). Therefore, I was tasked with finding a method which
...
interaction only approaches zero. Even at 6 \AA, the predicted binding energy of
DMA in water with \ch{Na^+} and \ch{NaCl} are 1.1 and 0.7 \kcalmol,
...
with literature that studies the Hofmeister series, where it has been shown that
biologically relevant cations are only able to influence their immediate
...
to nate of the IPs, DFT-based methods prefer to delocalize the charge, resulting
in non-physical binding between DMA and \ch{Mg^2+}, but not for \ch{Na+}. A
possible resolution to this is to use a constrained DFT method which enables one
to specify atomic occupancies,\cite{Melander2016} however, this technique is not
currently available in most common quantum chemical packages.
...
barrier may be possible using \ch{MgCl2}, however given the difficulties with Mg
in general, I elected to focus only on \ch{NaCl} for future considerations.
...
cis-DMA BDE increases with \ch{NaCl}, which is due an hydrogen-bond like
...
closely related formamide constributes about 30\% to the overall resonance
...
decreasing the effective BDE. This is also evindence by the decrease in the
...
$\pi$-system. However, that due to steric repulsion, the $\alpha$-radicals of
DIA cannot lie directly in the $\pi$-plane. As such the $\alpha$-\ch{C-H} BDEs
of DIA are greater than those of DMA by 2-3 \kcalmol, and are closer to the
$\beta$-\ch{C-H} BDEs than perhaps expected. The effects of sodium binding to
the amidic oxygen are almost nil for the $\beta$-trans C-H bond of DIA, however
there is a significant decrease in the $\beta$-cis \ch{C-H} BDE.
...
stabilizes the radical complex and thus decrease the effective BDE, however this
interaction is likely not possible in the TS structure and is not expected to
contribute to the effective BDE in a HAT reaction.
...
\ch{C-H} bonds $\alpha$ to electron-rich species which hyperconjugatively
overlap with the $\sigma^*$ anti-bonding orbital, the complexation of non-redox
active metals increases the effective \ch{C-H} bond strength. However, if the
\ch{C-H} bond is adjacent to an electron-poor centre, such as the carbon of a
carbonyl, metal complexation actually decreases the bond strength slightly by
stabilizing the carbon-centred radical.  In the context of HAT reaction barrier
heights, increasing the effective \ch{C-H} bond strength should decrease the
reaction-rate slightly by destabilizing the TS complex. However, there are other
important factors to consider such as how the metal effects to dipole moment in
...
chemo-protective effect by increasing the effective BDE of an adjacent \ch{C-H}
bond.
...
partial in the TS structures excluding \ch{NaCl} (ca. $0.03--0.05$ \AA).
...
DMA to \ch{NaCl}. As a result, there is less electron density available for
...
any PCET type orbital intereactions, charge separation between DMA and \cumo\ in
the TS structure may be considered as a partial ionization of the the hydrogen
atom. Therefore, by increasing charge separation in the TS structure, it becomes
easier to abstract the hydrogen atom as there is an increases the
...
separated reactants (or complexed DMA-NaCl) and were calculated at the
...
agreement of HAT reaction between DMA and \bno\ and \cumo. For abstraction by
\ch{HO^.}, the complexation of \ch{NaCl} to DMA increases the estimated free
energy reaction barriers across the board by 4.2--7.7 \kcalmol. This suggests
that if \ch{Na^+} interacts closely with DMA in vivo, then non-redox active
metals may have a chemo-protective effect against hydrogen abstraction by
\ch{HO^.}.
...
change in barrier height is calculated relative to the same abstraction site
without the inclusion of \ch{NaCl}. All barrier heights are relative to
separated reactants (or complexed DIA-NaCl) and were calculated at the
M05-2X-SMD(MeCN)/6-311+G(2d,2p)//M05-2X/6-31+G$^{**}$ level of theory.
$^*$Indicates estimated barrier based on ``guess'' TS structure.}
\label{tab:dia-cumo}
  \begin{tabular}{l l c c}
...
is not significantly effected by the addition of metal
...
with metal salts. I am interested in how metal salts effect the HAT reactivity
...
performed computational studies to determined if metal cations could effect this
...
barrier height is calculated relative to the same reaction without the inclusion
of \ch{NaCl}. All barrier heights are relative to separated reactants (or
complexed DMSO-NaCl) and were calculated at the
M05-2X-SMD(MeCN)/6-311+G(2d,2p)//M05-2X/6-31+G$^{**}$ level of theory.}
\label{tab:dmso-dG}
\begin{tabular}{l c c}
...
\ch{Na} will bind) should not significantly effect the \ch{C-H} bond strengths
...
heights are relative to separated reactants (or complexed HMPA/TBPO-NaCl) and
were calculated at the M05-2X-SMD(MeCN)/6-311+G(2d,2p)//M05-2X/6-31+G$^{**}$
level of theory. $^*$Indicates estimated barrier based on ``guess'' TS
structure.}
\label{tab:hmpa-tbpo}
\begin{tabular}{l c c}
Reaction   &  $\Delta G(H)^\ddagger$ &  $\Delta \Delta G(H)^\ddagger$ \\
...
of the \ch{CH2O^.} moiety of \bno\ of about $30--45^\circ$, while the TS in
...
effected, while it increases by about 1.5 \kcalmol\ for \bno. This is likely due
