...
starting point is the partition function $Q$,\bibnote{Note that $Q$ is actually
a function of the number of moles, volume of a system, and temperature. This
description, denoted as $Q(N,V,T)$, has been omitted for simplicity.} from which
...
\begin{equation}
  r = \frac{1}{c}\frac{d \text{C}}{dt} = \frac{1}{d}\frac{d \text{D}}{dt} = k[A]^\alpha [B]^\beta
\end{equation}

\noindent where $k$ is the rate constant, $t$ is time, $A, B, C$, and $D$ are
chemical species with stoichiometric coefficients $a, b, c$, and $d$, $\alpha$
and $\beta$ are the order of the reaction with respect to reactants A and B, and
$k$ is the rate constant. Computational chemistry is in general, not useful for
