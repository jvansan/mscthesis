...
to the strength of the breaking bond: $\Delta H =$ BDE(\ch{C-H}) $-$
BDE(\ch{CumO-H}). If the relationship holds for a series of related HAT
...
benzylic \ch{C-H} and \ch{C-OO^.} bond strengths with experimentally-measured
HAT rate constants and \ch{O2} addition rate constants, respectively. BEP plots
...
correlation ($R^2 = 0.82$) for \ch{C-H} bonds. This demonstrates that factors
...
reactions involving the abstraction of \ch{C-H} bonds by \cumo\ under the same
...
and cyclic amines and ethers, and substrates with allylic or benzylic \ch{C-H}
...
method.} Therefore, HAT reactions involving \cumo\ and the organic substrates of
...
methane as the reference \ch{C-H} bond model, the BDE of toluene should reflect
...
\ch{C-H} bond: one in which the incipient radical is delocalized into a
...
\end{tabularx}
  ...
  \cumo\ and substrates against weakest literature \ch{C-H} BDEs of the
  substrates. BDEs for dimethyl sulfoxide and hexamethylphosphoramide are from
  Ref.~\protect\citenum{Salamone2012}, while all other BDEs are from
  Ref.~\protect\citenum{Luo2002}. Points 21--26 and 29--30 were excluded from
...
set may be described as a comprehensive test of these methods for \ch{C-H} BDEs.
...
allylic/benzylic \ch{C-H} bonds, and another for alkyl \ch{C-H} bonds. However,
there remains a considerable amount of scatter in the data, thus correlation of
the expected BEP relations is poor. For the allylic/benzylic series of \ch{C-H}
...
BEP plot for the abstraction of \ch{C-H} bonds from models for unsaturated fatty
acids. Most of the rate constants used in the work of Pratt et al. are for the
abstraction of \ch{C-H} by peroxyl radicals, which were obtained through an
...
The alkyl \ch{C-H} BDEs show very weak correlation with \cumo\ HAT rate
...
adamantane), hydrogen bond donating (H-Bond Donors) species, and other \ch{C-H}
bonds with heteroatom neighbours (Het. Neighbours). Doing so appears to reveal
one reasonably well-correlated trend for \ch{C-H} bonds with heteroatom
...
and 2,2-dimethylbutane, as well as the secondary and tertiary \ch{C-H} positions
of adamantane. There may be a separate correlation for each of these, however
...
not allow for HAT to occur without some subsequent rearrangement. In general,
there are no evident reasons on the basis of group-additivity based arguments
that explain the poor correlations observed. Thus, the lack of simple
relationships is perhaps evidence against the validity of the BEP principle.
However, before making any conclusions, we must consider if there are any
explanations that arise from examining the transition state structures.
...
moieties and the \ch{C-H} of the methyl of \cumo. In all but two cases, this
...
other hand, the \ch{C(CH3)2O^.} of \cumo\ could rotate to accommodate a
...
\begin{enumerate}
  ...
  abstraction reactivity of \cumo\ (or \ch{$t$-BuO^.}). Therefore, I suggest
