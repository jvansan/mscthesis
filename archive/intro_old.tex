...
In organic synthesis, the replacement of specific \ch{C-H} bonds (C-H bond
...
methods for \ch{C-H} functionalisation have been achieved. The most commonly used
technique is the hydroxylation of \ch{C-H} bonds, which then serve as a synthetic
handle. Transition metal chemistry is now typically employed to achieve \ch{C-H} bond
functionalisation. This often involves highly reactive metal-oxo intermediates
responsible for triggering \ch{C-H} bond cleavage, with an inorganic HAT reaction
...
from our colleagues in Rome has demonstrated that for \ch{C-H} bond HAT from
...
\toprule
  ...
  \end{tabular}
  \caption[Summary of second-order rate constants for HAT from \ch{C-H} bonds for
  hydrogen bond accepting substrates from the cumyloxyl radical.]{Second-order
    rate constants ($k_H$, \Ms) for HAT from \ch{C-H} bonds of hydrogen bond
...
into \ch{C-H} $\sigma^*$ antibonding orbitals. Hyperconjugation has a net stabilising
effect, however it decreases the strength of \ch{C-H} bonds by accepting electron
density into a \ch{C-H} $\sigma^*$ orbital. For the species listed in~\ref{tab:kse},
abstraction occurs primarily from \ch{C-H} bonds adjacent to a heteroatom or $\pi$
system. Hyperconjugative overlap between the heteroatom and \ch{C-H} $\sigma^*$
antibonding orbitals in these species is decreased by the formation of a
hydrogen bond with solvent, effectively increasing the strength of the \ch{C-H} bond
...
theoretically determined \ch{C-H} and \ch{C-OO^.} bond strengths with experimentally
...
fatty acid models show good correlation for both the \ch{C-H} bonds and \ch{C-OO^.}
...
important. HAT is favoured from \ch{C-H} bonds which are electron rich, or
...
C-H BDEs. Cyclohexane has a \ch{C-H} BDE = 99.5 \kcalmol and $k_H$ = $9.2\times10^4$
\Ms, while acetone has \ch{C-H} BDE of 96.0 \kcalmol and $k_H$ = $ < 2\times10^3$
...
others.\cite{Finn2004,Salamone2011,Pischel2001,Griller1981,Bietti2011,
  Salamone2012,Malatesta1982,Salamone2014} Although a \ch{C-H} bond may be weaker
...
non-redox active metal cations on HAT from \ch{C-H} bonds of cyclohexadiene (CHD),
...
In general, metal cation interactions lead to deactivation of \ch{C-H} bonds and
...
bond $\sigma^*$ population, thus increasing the effective \ch{C-H} bond strength and
...
strongly with substrates than \cumo, thus \ch{C-H} bond deactivation is observed due
...
models.\cite{Salamone2015a} As such the deactivation of \ch{C-H} bonds in amides, as
...
% aliphatic \ch{C-H} bonds as a whole, and may (or probably doesn't now?) apply to all
% other alkyl \ch{C-H} bonds (Chapter \ref{ch:bde}); and non-redox active metal
...
% \textbf{Chapter 5}
% \begin{itemize}
% \item To obtain highly-accurate bond dissociation enthalpies for \ch{C-H} bonds for
...
%   determine if there exists two linear relationships for allylic and alkyl \ch{C-H}
%   bonds.
% \end{itemize}
